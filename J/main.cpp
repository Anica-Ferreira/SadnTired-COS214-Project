...
void testCommand() {
    ...
    // Extension: Specialized and chain commands
    cout << "\n--- Chained Commands ---\n";
    WaterCommand* waterCmd = new WaterCommand();
    PruneCommand* pruneCmd = new PruneCommand();

    // Chain command chains multiple commands together
    ChainCommand* chain = new ChainCommand();
    chain->addCommand(waterCmd);  // Add watering to sequence
    chain->addCommand(pruneCmd);  // Add pruning to sequence
    chain->execute("Cactus");  // Execute both commands in order

    // Cleanup - ChainCommand destructor deletes its contained commands
    delete chain;
    delete action;
